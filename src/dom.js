 ...
 */
function queryExists(query = 'query', existsFunction = () => {}) {
    ...
    queryExists(
        ...
        () => queryToArray(query).map((dom) => {
            ...
            if (typeof dom[event] == 'function') {
                let newEventFunction = dom[event].bind({});
                eventWrapperFunction = () => {
                    try {
                        newEventFunction();
                    } catch (e) {
                        console.error('Failed to run: ' + newEventFunction);
                    }
            ...
            dom[event] = eventWrapperFunction;
        })
    );
}
